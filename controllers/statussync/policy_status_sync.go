...
// convention. If the template's status field is not found, is not owned by the policy,
// or is not in the correct format, it returns an empty list.
func getEventsInTemplate(
	tmplUnstruct *unstructured.Unstructured, policyName string,
) []policiesv1.ComplianceHistory {
	if tmplUnstruct == nil || !templateOwnedByPolicy(tmplUnstruct, policyName) {
...
// templateOwnedByPolicy checks whether the template object belongs to the given policy
func templateOwnedByPolicy(obj metav1.Object, policyName string) bool {
	refName := ""
	...
	for i, policyT := range instance.Spec.PolicyTemplates {
		...
		if err != nil {
			...
			if !ok || tmplGVK == nil {
				...
				eventForPolicyMap[tName] = append(
					eventForPolicyMap[tName], getEventsInTemplate(tmplUnstruct, policyObjID.Name)...,
				)

				if !templateOwnedByPolicy(tmplUnstruct, policyObjID.Name) {
					// If the template is not owned by the policy, skip this DPT.
					existingDPTs = nil
				}
			}
		}
