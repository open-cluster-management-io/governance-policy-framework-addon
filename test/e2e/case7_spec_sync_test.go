...
const (
	case7PolicyName string = "default.case1-test-policy"
	case7PolicyYaml string = "../resources/case1_spec_sync/case1-test-policy.yaml"
)

var _ = Describe("Test spec sync", func() {
	BeforeEach(func() {
		By("Creating a policy on hub cluster in ns:" + testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", case7PolicyYaml, "-n", testNamespace,
	...
	AfterEach(func() {
		...
		_, _ = utils.KubectlWithOutput("delete", "-f", case7PolicyYaml, "-n", testNamespace,
			"--kubeconfig=../../kubeconfig_hub")
