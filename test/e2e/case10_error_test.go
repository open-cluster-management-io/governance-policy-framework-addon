...
var _ = Describe("Test error handling", func() {
	...
	It("should not override remediationAction if doesn't exist on parent policy", func() {
		By("Creating ../resources/case10_template_sync_error_test/remediation-action-not-exists.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f",
			"../resources/case10_template_sync_error_test/remediation-action-not-exists.yaml", "-n", testNamespace)
		...
		By("Patching ../resources/case10_template_sync_error_test/remediation-action-not-exists2.yaml on managed cluster in ns:" +
			testNamespace)
		_, err = utils.KubectlWithOutput("apply", "-f",
			"../resources/case10_template_sync_error_test/remediation-action-not-exists2.yaml", "-n", testNamespace)
		Expect(err).Should(BeNil())
	...
	It("should generate decode err event", func() {
		By("Creating ../resources/case10_template_sync_error_test/template-decode-error.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/template-decode-error.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
	...
	It("should generate missing name err event", func() {
		By("Creating ../resources/case10_template_sync_error_test/template-name-error.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/template-name-error.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
	...
	It("should generate mapping err event", func() {
		By("Creating ../resources/case10_template_sync_error_test/template-mapping-error.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/template-mapping-error.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
		By("Checking for event with decode err on managed cluster in ns:" + testNamespace)
		Eventually(
	...
	It("should generate duplicate policy template err event", func() {
		By("Creating ../resources/case10_template_sync_error_test/working-policy-duplicate.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/working-policy.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
		...
		_, err = utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/working-policy-duplicate.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
	...
	It("should create other objects, even when one is invalid", func() {
		By("Creating ../resources/case10_template_sync_error_test/middle-template-error.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/middle-template-error.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
	...
	It("should remove the complianceState on a template only after an error is resolved", func() {
		By("Creating ../resources/case10_template_sync_error_test/working-policy.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/working-policy.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
	...
	It("should throw a noncompliance event if a non-configurationpolicy uses a hub template", func() {
		By("Creating ../resources/case10_template_sync_error_test/non-config-hubtemplate.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/non-config-hubtemplate.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
	...
	It("should throw a noncompliance event if the template object is invalid", func() {
		By("Creating ../resources/case10_template_sync_error_test/invalid-severity-template.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/invalid-severity-template.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
	...
	It("should not throw a noncompliance event if the policy-templates array is empty", func() {
		By("Creating ../resources/case10_template_sync_error_test/empty-templates.yaml on managed cluster in ns:" +
			testNamespace)
		_, err := utils.KubectlWithOutput("apply", "-f", "../resources/case10_template_sync_error_test/empty-templates.yaml",
			"-n", testNamespace)
		Expect(err).Should(BeNil())
